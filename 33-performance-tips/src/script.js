...
/***   Renderer   ***/
const renderer = new THREE.WebGLRenderer({ canvas: canvas, powerPreference: 'high-performance', antialias: true });
...
// scene.add(cube, torusKnot, sphere, floor);
...
// Tip 10: Lights
// (directionalLight.shadow.camera.top = 3), (directionalLight.shadow.camera.right = 6);
// (directionalLight.shadow.camera.left = -6), (directionalLight.shadow.camera.bottom = -3);
...
// (floor.castShadow = false), (floor.receiveShadow = true);
...
// Tip 18
// for (let i = 0; i < 50; i++) {
//   const geometry = new THREE.BoxGeometry(0.5, 0.5, 0.5), material = new THREE.MeshNormalMaterial();
//   const mesh = new THREE.Mesh(geometry, material);
//   mesh.position.set((Math.random() - 0.5) * 10, (Math.random() - 0.5) * 10, (Math.random() - 0.5) * 10);
//   mesh.rotation.x = (Math.random() - 0.5) * Math.PI * 2, mesh.rotation.y = (Math.random() - 0.5) * Math.PI * 2;
//   scene.add(mesh);
// }
...
//   geometry.rotateX((Math.random() - 0.5) * Math.PI * 2), geometry.rotateY((Math.random() - 0.5) * Math.PI * 2);
...
//   matrix.makeRotationFromQuaternion(quaternion), matrix.setPosition(position);
...
const shaderMaterial = new THREE.ShaderMaterial({
  ...
  vertexShader: `
        ...
        void main() {
            ...
            // vUv = uv;
            color = mix(vec3(1.0, 0.1, 0.1), vec3(0.1, 0.0, 0.5), max(texture2D(uDisplacementTexture, uv).r, .025));
        }`,
  fragmentShader: `
        // uniform sampler2D uDisplacementTexture;
        // varying vec2 vUv;
        ...
        void main() {
            ...
            // if(elevation < 0.25) { elevation = 0.25; }
            // float elevation = max(texture2D(uDisplacementTexture, vUv).r, .025);
            ...
            // gl_FragColor = vec4(mix(depthColor, surfaceColor, elevation), 1.0);
